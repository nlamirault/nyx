...
import (
	...
	"github.com/nlamirault/nyx/keyboard"
	"github.com/nlamirault/nyx/layout"
)
...
func New() (*Compositor, error) {
	layout, err := layout.New("vertical")
	if err != nil {
		return nil, err
	}
	return &Compositor{
...
func (c *Compositor) startInteractiveAction(view wlc.View, origin wlc.Point) bool {
	glog.V(2).Info("Start interactive action")
	if c.action != nil {
		return false
	}
...
func (c *Compositor) stopInteractiveAction() {
	glog.V(2).Info("Stop interactive action")
	if c.action == nil {
		return
	}
...
func (c *Compositor) ViewRequestMove(view wlc.View, origin *wlc.Point) {
	glog.V(2).Info("View request move")
...
func (c *Compositor) ViewRequestResize(view wlc.View, edges uint32, origin *wlc.Point) {
	glog.V(2).Info("View request resize")
...
func (c *Compositor) KeyboardKey(view wlc.View, time uint32, modifiers wlc.Modifiers, key uint32, state wlc.KeyState) bool {
	glog.V(2).Info("Keyboard action")
	sym := xkb.KeySym(wlc.KeyboardGetKeysymForKey(key, nil))
	if state == wlc.KeyStatePressed {
		c.Keyboard.PressKey(sym, key)
	} else {
		c.Keyboard.ReleaseKey(sym, key)
	}
